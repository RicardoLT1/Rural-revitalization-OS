...
import { createUser, fetchUsers, resetUserPassword, setUserEnabled, updateUser } from '../api/admin'
...
const filtered = computed(() => rows.value.filter(item => {
  ...
  return (!term || `${item.username} ${item.displayName}`.toLowerCase().includes(term)) && (role.value === 'ALL' || item.role === role.value)
}))

async function load() {
  loading.value = true; error.value = ''
  try { rows.value = await fetchUsers() }
...
<template>
  <div class="business-page">
    <section class="page-intro"><div><p>账号与访问边界</p><h2>用户与权限</h2></div><button class="primary-button compact-primary" type="button" @click="open()"><Plus :size="17" />新增用户</button></section>
    <div v-if="notice" class="toast-notice"><ShieldCheck :size="16" />{{ notice }}</div>
    <section class="business-toolbar"><div class="filter-group"><label><Search :size="16" /><input v-model="keyword" placeholder="搜索账号或姓名" /></label><select v-model="role"><option value="ALL">全部角色</option><option value="USER">普通用户</option><option value="STAFF">工作人员</option><option value="ADMIN">管理员</option></select></div></section>
    <PageState :loading="loading" :error="error" :empty="!filtered.length" empty-text="没有符合条件的用户" @retry="load"><section class="table-panel"><div class="table-scroll"><table><thead><tr><th>账号</th><th>姓名</th><th>角色</th><th>状态</th><th>村域</th><th>操作</th></tr></thead><tbody><tr v-for="item in filtered" :key="item.id"><td><strong>{{ item.username }}</strong><small>ID {{ item.id }}</small></td><td>{{ item.displayName }}</td><td><span class="role-badge">{{ item.role }}</span></td><td><span class="account-state" :class="{ disabled: !item.enabled }"><i />{{ item.enabled ? '启用' : '停用' }}</span></td><td>{{ item.villageId }}</td><td><div class="row-buttons"><button class="table-action" type="button" @click="open(item)"><Pencil :size="14" />编辑</button><button class="table-action" type="button" @click="resetPassword(item)"><KeyRound :size="14" />重置密码</button><button class="table-action" :class="item.enabled ? 'reject' : 'approve'" type="button" @click="toggle(item)"><UserX v-if="item.enabled" :size="14" /><UserCheck v-else :size="14" />{{ item.enabled ? '停用' : '启用' }}</button></div></td></tr></tbody></table></div></section></PageState>

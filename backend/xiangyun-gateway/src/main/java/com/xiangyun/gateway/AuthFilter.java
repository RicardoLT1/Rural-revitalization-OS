...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public AuthFilter(ReactiveStringRedisTemplate redisTemplate, @Value("${xiangyun.jwt.secret}") String secret) {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return redisTemplate.opsForValue().get("login:token:" + payload.jti())
                .flatMap(value -> {
                    ...
                    return chain.filter(withUser(exchange, traceId, payload));
                })
    ...
    private ServerWebExchange withTrace(ServerWebExchange exchange, String traceId) {
        ServerHttpRequest request = exchange.getRequest().mutate()
                .headers(headers -> headers.set(SecurityHeaders.TRACE_ID, traceId))
                .build();
        return exchange.mutate().request(request).build();
    }

    private ServerWebExchange withUser(ServerWebExchange exchange, String traceId, TokenPayload payload) {
        ServerHttpRequest request = exchange.getRequest().mutate()
                .headers(headers -> {

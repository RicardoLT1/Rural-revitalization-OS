...
Page({
  ...
  onShow() { if (!this.data.isLoading) this.loadApplications(); },
  onPullDownRefresh() { this.loadApplications().finally(() => wx.stopPullDownRefresh()); },
  onRetry() { this.loadApplications(); },
  onStatusTap(event) {
    const value = event.currentTarget.dataset.value;
    if (value) this.setData({ activeStatus: value }, () => this.applyFilter());
  },
  ...
  applyFilter() {
    ...
    const myApplications = activeStatus === 'ALL' ? source : source.filter((item) => item.status === activeStatus);
    this.setData({ myApplications, pageState: myApplications.length ? PageState.Ready : PageState.Empty });
  },
  onApplicationTap(event) {
    const processId = event.currentTarget.dataset.processid;
    if (processId) goProcessDetail(processId);
  }
});